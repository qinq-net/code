...
def listen(IN,getstop,timein=0.05,timeout=3,release=True):
    while True:
        if getstop(): return getstop
        if GPIO.input(IN):
            ...
            if GPIO.input(IN):
                ...
                while GPIO.input(IN):
                    if getstop(): return getstop
def listen2(IN,timeout=1,release=False):
    calltime=time.time()
    while time.time() - calltime < timeout:
        if GPIO.input(IN):
            if not release: return (time.time()-calltime, True)
            if GPIO.input(IN):
                starttime=time.time()
                while time.time() - starttime < timeout:
                    if not GPIO.input(IN): return (starttime-calltime, time.time()-starttime)
    else: return False
...
class thread2(thread):
    def __init__(self,IN,IN2,OUT):
        thread.__init__(self,IN,OUT,mode=4)
        self.IN2=IN2
    ...
    def target(self):
       while True:
           GPIO.output(self.OUT,GPIO.HIGH)
           self.flag=True
           listen(self.IN,self.getstop,timein=0.05)
           if self.stopflag: return
           else:
               answer=listen2(self.IN2)
